...
class GameContainerFactory {
  ...
  def createGameContainer(player1Id: String, player2Id: String) = {
    ...
    val player1 = Player(Rocket(player1Pos, player1Rotation, Vector2D(0, 0), phys.rocketRadius), alive = true)

    val player2X = player1X * 3
    val player2Pos = Vector2D(player2X, playerY)
    val player2Rotation = (planetPosition - player1Pos).normalise
    val player2 = Player(Rocket(player2Pos, player2Rotation, Vector2D(0, 0), phys.rocketRadius), alive = true)

    val model = GameModel(planet, Vector(player1, player2), Set())
